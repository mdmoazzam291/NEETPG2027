...
async function loadPyq(page){
  await page.goto('/');
  await expect.poll(()=>page.evaluate(()=>typeof app!=='undefined' ? app.questions.length : 0)).toBeGreaterThanOrEqual(220);
...
test('PYQ Intelligence view renders and launches repeated-concept practice', async ({page})=>{
  ...
  await expect(page.locator('#pyqTotal')).toHaveText('120');
...
test('PYQ year filters and repeat filters constrain practice data', async ({page})=>{
  ...
  expect(result.recentCount).toBe(60);
