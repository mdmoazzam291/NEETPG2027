...
function boot(){const legacy=readPrefs();if(Object.prototype.hasOwnProperty.call(legacy,'examDate')){delete legacy.examDate;writePrefs(legacy)}setTimeout(panel,900);document.addEventListener('click',e=>{if(e.target.closest('[data-view="dashboard"],[data-v4-target="dashboard"],#refreshDashboard'))setTimeout(panel,120)});window.addEventListener('storage',e=>{if(e.key===KEY)panel()});window.NEETPG_PHASE12={KEY,profileGoal,setDailyGoal,savePlan,goalProgress,srsCalendar,balanceSrs,dailyMix,studyPlan,microSession,rollingPlan,reminderEligibility,startPlan,startToday,startMicro,panel}}
