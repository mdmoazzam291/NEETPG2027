...
const DAY=86400000;
const DEFAULTS={dailyGoal:50,examDate:''};
...
const validExamDate=value=>/^\d{4}-\d{2}-\d{2}$/.test(value||'')?value:'';
...
function setExamDate(value){const p=readPrefs();p.examDate=validExamDate(value);writePrefs(p);panel();return p.examDate}
function savePlan(goalValue,examValue){const goal=clamp(Math.round(Number(goalValue)||50),1,1000),examDate=validExamDate(examValue),p=readPrefs();p.dailyGoal=goal;p.examDate=examDate;writePrefs(p);syncCloudGoal(goal);panel();try{window.dispatchEvent(new CustomEvent('neetpg:plan-updated',{detail:{dailyGoal:goal,examDate}}))}catch{}return{dailyGoal:goal,examDate}}
function examCountdown(){const value=readPrefs().examDate;if(!value)return{date:null,days:null,status:'Set your exam date to activate countdown planning.'};const target=new Date(`${value}T00:00:00`);if(Number.isNaN(target.getTime()))return{date:null,days:null,status:'Invalid exam date'};const days=Math.ceil((target.getTime()-Date.now())/DAY);return{date:value,days,status:days<0?'Exam date has passed':days===0?'Exam day':`${days} day${days===1?'':'s'} remaining`}}
...
function injectStyles(){if(document.querySelector('#phase12Styles'))return;const s=document.createElement('style');s.id='phase12Styles';s.textContent=`#v12Planner{margin:16px 0}.p12-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.p12-metric{padding:12px;border:1px solid var(--line,#e5e7eb);border-radius:14px;background:var(--surface2,#f8fafc)}.p12-metric b{display:block;font-size:21px}.p12-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.p12-calendar{display:grid;grid-template-columns:repeat(7,1fr);gap:6px;margin-top:10px}.p12-day{padding:8px 4px;border-radius:10px;text-align:center;background:var(--surface2,#f8fafc);font-size:11px}.p12-day b{display:block;font-size:15px}.p12-config{display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:end;margin-top:12px}@media(max-width:800px){.p12-grid{grid-template-columns:1fr 1fr}.p12-config{grid-template-columns:1fr 1fr}.p12-config button{grid-column:1/-1}.p12-calendar{grid-template-columns:repeat(4,1fr)}}`;
