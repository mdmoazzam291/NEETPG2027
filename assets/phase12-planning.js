...
function writePrefs(p){try{localStorage.setItem(KEY,JSON.stringify({...DEFAULTS,...p}));window.dispatchEvent(new CustomEvent('neetpg:data-change'))}catch{toast('Study plan could not be saved.')}}
...
function panel(){
  const layout=document.querySelector('#view-plan'),host=layout?.querySelector('#v12Planner[data-dashboard-slot="planner"]');if(!host)return false;injectStyles();host.classList.remove('hidden');
...
function boot(){const legacy=readPrefs();if(Object.prototype.hasOwnProperty.call(legacy,'examDate')){delete legacy.examDate;writePrefs(legacy)}const schedulePanel=()=>panel();window.addEventListener('neetpg:dashboard-render',schedulePanel);window.addEventListener('neetpg:route-change',e=>{if(e.detail?.view==='plan')panel()});window.addEventListener('neetpg:data-change',()=>{if(document.querySelector('#view-plan.active'))panel()});window.addEventListener('storage',e=>{if(e.key===KEY)schedulePanel()});schedulePanel();window.NEETPG_PHASE12={KEY,profileGoal,setDailyGoal,savePlan,goalProgress,srsCalendar,balanceSrs,dailyMix,studyPlan,microSession,rollingPlan,reminderEligibility,startPlan,startToday,startMicro,panel,schedulePanel}}
