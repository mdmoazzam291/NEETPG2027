...
function panel(){
  const dash=document.querySelector('#view-dashboard'),layout=dash?.querySelector('.v4-dashboard');if(!layout)return false;injectStyles();let host=document.querySelector('#v12Planner');if(!host){host=document.createElement('div');host.id='v12Planner';host.className='card';const countdown=layout.querySelector('#v4ExamCountdown');countdown?countdown.insertAdjacentElement('afterend',host):layout.prepend(host)}
...
function boot(){const legacy=readPrefs();if(Object.prototype.hasOwnProperty.call(legacy,'examDate')){delete legacy.examDate;writePrefs(legacy)}const schedulePanel=(tries=0)=>{if(panel()||tries>=25)return;setTimeout(()=>schedulePanel(tries+1),100)};setTimeout(()=>schedulePanel(),150);document.addEventListener('click',e=>{if(e.target.closest('[data-view="dashboard"],[data-v4-target="dashboard"],#refreshDashboard'))setTimeout(()=>schedulePanel(),80)});window.addEventListener('storage',e=>{if(e.key===KEY)schedulePanel()});window.NEETPG_PHASE12={KEY,profileGoal,setDailyGoal,savePlan,goalProgress,srsCalendar,balanceSrs,dailyMix,studyPlan,microSession,rollingPlan,reminderEligibility,startPlan,startToday,startMicro,panel,schedulePanel}}
