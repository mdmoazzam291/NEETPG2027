(() => {
  ...
  async function clearActiveSession(sessionId){
    if(!cloud.user)return;
    let query=cloud.client.from('active_sessions').delete().eq('user_id',cloud.user.id);
  ...
  async function pushActiveSession(){
    ...
    if(clearIntent){
      if(!remote || remote.sessionId!==clearIntent.sessionId){
        ...
        await clearActiveSession(clearIntent.sessionId);
  ...
  cloud.clearActiveSession=clearActiveSession;
